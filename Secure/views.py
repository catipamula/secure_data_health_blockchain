...
def updateRevenue(value):
    db_connection = pymysql.connect(host='localhost',port = 3306,user = 'root', password = 'root', database = 'SecuringData',charset='utf8')
    ...
    if connection:
        try:
            with connection.cursor() as cursor:
                sql = "UPDATE patients SET revenue=revenue+0.5 WHERE patient_id=%s"
...
def PatientDataAccess(request):
    if request.method == 'POST':
      ...
      with con:
          cur = con.cursor()
          cur.execute("select * FROM patients where problem_desc like '%"+search+"%' OR blockchain_hash LIKE '%"+search+"%'" )
        #   cur.execute("select * FROM patients where problem_desc like '%"+search+"%' OR blockchain_hash LIKE '%"+search+"%'" )
          rows = cur.fetchall() 
          for row in rows: 
             arr = row[5].split(" ")
